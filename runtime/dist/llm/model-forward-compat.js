...
const ANTHROPIC_SONNET_TEMPLATE_IDS = ["claude-sonnet-4-5", "claude-sonnet-4.5"];
const ANTHROPIC_OPUS_46_ID = "claude-opus-4-6";
const ANTHROPIC_OPUS_46_DOT_ID = "claude-opus-4.6";
const ANTHROPIC_OPUS_TEMPLATE_IDS = ["claude-opus-4-5", "claude-opus-4.5"];
// ─── OpenAI ───────────────────────────────────────────────────────────────────
const OPENAI_GPT_53_ID = "gpt-5.3";
const OPENAI_GPT_53_TEMPLATE_IDS = ["gpt-5.2"];
const OPENAI_GPT_54_ID = "gpt-5.4";
const OPENAI_GPT_54_TEMPLATE_IDS = ["gpt-5.2"];
// ─── Helpers ──────────────────────────────────────────────────────────────────
function cloneFirstTemplateModel(params) {
    for (const templateId of [...new Set(params.templateIds)].filter(Boolean)) {
        try {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const template = getModel(params.provider, templateId);
            if (!template)
                continue;
...
function resolveAnthropicForwardCompat(modelId, dashId, dotId, templateIds) {
...
function resolveOpenAIForwardCompat(modelId, targetId, templateIds) {
 ...
 */
export function resolveForwardCompatModel(provider, modelId) {
    const normalizedProvider = provider.trim().toLowerCase();
    const trimmedModelId = modelId.trim();
    if (normalizedProvider === "anthropic") {
        return (resolveAnthropicForwardCompat(trimmedModelId, ANTHROPIC_SONNET_46_ID, ANTHROPIC_SONNET_46_DOT_ID, ANTHROPIC_SONNET_TEMPLATE_IDS) ??
            resolveAnthropicForwardCompat(trimmedModelId, ANTHROPIC_OPUS_46_ID, ANTHROPIC_OPUS_46_DOT_ID, ANTHROPIC_OPUS_TEMPLATE_IDS));
    }
    if (normalizedProvider === "openai") {
        return (resolveOpenAIForwardCompat(trimmedModelId, OPENAI_GPT_53_ID, OPENAI_GPT_53_TEMPLATE_IDS) ??
