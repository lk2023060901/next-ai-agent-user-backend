...
function normalizeStatus(status: unknown, fallback: "success" | "failure"): "success" | "failure" | "partial" {
  const normalized = String(status ?? "").trim().toLowerCase();
  if (ALLOWED_PLUGIN_USAGE_STATUSES.has(normalized)) {
    return normalized as "success" | "failure" | "partial";
  }
  return fallback;
}
...
function extractPluginUsageMeta(result: unknown): {
  eventType?: string;
